...
const items = [
    ...
    { id: 41, name: 'Boeing 747', price: 148000000, image: 'https://neal.fun/spend/images/boeing-747.jpg' },
...
// Initial values
let moneyRemaining = 100000000000; // $100 billion
let moneySpent = 0;
const inventory = {}; // To track purchases
...
function updateMoneyDisplay() {
    document.getElementById('money-remaining').textContent = formatNumber(moneyRemaining);
    document.getElementById('money-spent').textContent = formatNumber(moneySpent);
}

// Update button states
function updateButtonStates() {
    items.forEach(item => {
        const buyButton = document.querySelector(`.buy-button-${item.id}`);
        const sellButton = document.querySelector(`.sell-button-${item.id}`);
        
        // Disable buy button if not enough money
        if (buyButton) {
            buyButton.disabled = moneyRemaining < item.price;
        }
        
        // Disable sell button if no items to sell
        if (sellButton) {
            sellButton.disabled = !inventory[item.id] || inventory[item.id] === 0;
        }
    });
}
...
function buyItem(itemId) {
    ...
    if (moneyRemaining >= item.price) {
        // Update money
        moneyRemaining -= item.price;
        ...
        document.querySelector(`.quantity-${itemId}`).textContent = inventory[itemId];
        
        // Update money display and button states
        updateMoneyDisplay();
        updateButtonStates();
    }
}
...
function generateItemCards() {
    ...
    items.forEach(item => {
        ...
        const formattedPrice = item.price >= 1000 ? formatNumber(item.price) : item.price;
        ...
        itemCard.innerHTML = `
            ...
            <div class="item-controls">
                <button class="sell sell-button-${item.id}" onclick="sellItem(${item.id})" disabled>Sell</button>
                <span class="quantity quantity-${item.id}">0</span>
                <button class="buy buy-button-${item.id}" onclick="buyItem(${item.id})">Buy</button>
            </div>
        `;
